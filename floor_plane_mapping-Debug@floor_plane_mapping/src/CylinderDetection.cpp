...
static __forceinline double GetDistanceBetweenPoints(const pcl::PointXYZ& A, const pcl::PointXYZ& B)
{
...
protected:
	...
	void pc_callback(const sensor_msgs::PointCloud2ConstPtr msg) {
		...
		for(auto it = mapCirclesInPlaneSections.begin(); it!=mapCirclesInPlaneSections.end(); ++it)
		{
			...
			{
				if(GetDistanceBetweenPoints(it->second.first, itNext->second.first) < 0.05 && fabs(it->second.second-itNext->second.second) < 0.05)
				{
					...
					meanCenter.z += ((itNext->second.first.z + it->second.first.z)/2.0);
				...
				else
				{
					...
					meanCenter.z /= numSections;
					meanRadius /= numSections;
					if(numSections >= 3)
						// if(numSections < 3) Not enough acceptable sections to consider that there is a cylinder
					else
					{
						ROS_INFO("Cylinder found: Center (%.2f, %.2f, %.2f) Radius %.2f Bottom z = %.2f Top z = %.2f", meanCenter.x, meanCenter.y, meanCenter.z, dBottomCap, dTopCap);
					}
